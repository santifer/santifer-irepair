...
async function processImagesFromAirtable(baseAirtable, ruta) {
  ...
  base(baseAirtable)
    .select({
      view: 'ScriptImagenesReseñas',
    })
    .eachPage(async (records, fetchNextPage) => {
      for (const record of records) {
        const imageUrl = record.get('Foto')[0].url;
        const slug = record.get('imageUrlPrevia');
        console.log(`Processing record ID: ${record.getId()}`);
        console.log(`Record fields:`, record.fields);

        if (!imageUrl || !slug) {
          console.warn(
            `Missing fields for record ID: ${record.getId()}, imageUrl: ${imageUrl}, slug: ${slug}`
          );
          continue; // Saltar al siguiente registro si faltan campos
        }

        if (imageUrl && slug) {
          try {
            console.log(
              `Downloading image from URL: ${imageUrl}, for slug: ${slug}`
            );
...
async function updateAirtableRecord(recordId, imageUrl, baseAirtable) {
  console.log(
    `Updating Airtable record ID: ${recordId}, with imageUrl: ${imageUrl}`
  );
  try {
...
processImagesFromAirtable('Reseñas sincronizar Astro', 'res');
